# Hey, I'm Amrutha 👋

Most of my time is currently spent exploring **Machine Learning, Data Science, AI research, and backend development**. I enjoy taking ideas from research papers and turning them into working prototypes that solve real-world problems.

I'm a Computer Science student who is fascinated by one question:

### How can data help us understand complex systems and make better decisions?

That question is what pulled me into Machine Learning, AI research, analytics, and software development.

Right now, I'm exploring everything from predictive healthcare models to intelligent applications powered by large language models. Some days I'm reading research papers, some days I'm debugging code for hours, and some days I'm experimenting with datasets just to see what patterns emerge.


My goal is to become a Data Scientist and contribute to innovative solutions in healthcare, education, and emerging technologies through data-driven decision making.

---

## 🎯 Current Focus

- 📊 Data Science & Machine Learning
- 🤖 Artificial Intelligence Research
- 📈 Data Analysis & Visualization
- ☁️ Cloud Computing (AWS)
- 🧠 Deep Learning & Generative AI
- 🔬 Research Publications
- 🚀 Open Source Contributions

---

## 🛠️ Tech Stack

### Programming Languages
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![Java](https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=openjdk&logoColor=white)
![C](https://img.shields.io/badge/C-00599C?style=for-the-badge&logo=c&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)

### Data Science & Machine Learning
![TensorFlow](https://img.shields.io/badge/TensorFlow-FF6F00?style=for-the-badge&logo=tensorflow&logoColor=white)
![Scikit Learn](https://img.shields.io/badge/Scikit--Learn-F7931E?style=for-the-badge&logo=scikitlearn&logoColor=white)
![Pandas](https://img.shields.io/badge/Pandas-150458?style=for-the-badge&logo=pandas&logoColor=white)
![NumPy](https://img.shields.io/badge/NumPy-013243?style=for-the-badge&logo=numpy&logoColor=white)
![Matplotlib](https://img.shields.io/badge/Matplotlib-11557c?style=for-the-badge)
![Seaborn](https://img.shields.io/badge/Seaborn-4E79A7?style=for-the-badge)

### Web Development
![React](https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react)
![Node.js](https://img.shields.io/badge/Node.js-339933?style=for-the-badge&logo=nodedotjs&logoColor=white)
![Bootstrap](https://img.shields.io/badge/Bootstrap-563D7C?style=for-the-badge&logo=bootstrap)
![TailwindCSS](https://img.shields.io/badge/TailwindCSS-38B2AC?style=for-the-badge&logo=tailwind-css)

### Tools & Platforms
![AWS](https://img.shields.io/badge/AWS-232F3E?style=for-the-badge&logo=amazonaws)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker)
![Kubernetes](https://img.shields.io/badge/Kubernetes-326CE5?style=for-the-badge&logo=kubernetes)
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github)

---

## 🔥 Featured Projects

### 🧬 Human Kidney Digital Twin Simulation
Developed a research-driven framework for Human Digital Twins to enable predictive healthcare simulations.

**Highlights**
- Multi-scale healthcare data integration
- Machine Learning-driven predictions
- Precision medicine simulations
- Healthcare analytics and ethical AI considerations

**Tech:** Python, TensorFlow, Scikit-Learn, Pandas, Flask API, Streamlit

---

### 📚 AI Research Paper Assistant
An AI-powered platform that automates research paper analysis and summarization.

**Features**
- PDF text extraction
- AI-generated summaries
- Citation extraction
- Research insights generation

**Tech:** Python, Streamlit, Ollama, Sentence Transformers, PyPDF

---

### 🎤 AI Interview Practice System
An intelligent interview preparation platform powered by AI.

**Features**
- Technical & behavioral interview simulations
- Automated performance evaluation
- AI-generated feedback
- LLM integration

**Tech:** Python, Flask API, REST APIs, Local AI Models, Cloud LLMs

---

## 💼 Experience

### AWS Web & AI Intern
- Built backend modules for AI-powered applications
- Worked with AWS cloud services
- Automated deployment pipelines
- Collaborated using Git and GitHub workflows

---

## 🏆 Achievements

🏅 Top 10% ranking in multiple hackathons

📜 AWS Web & AI Internship Certification

🚀 Deployed multiple real-world AI and Web applications

💻 Active GitHub Contributor

📚 Continuous learner in AI, Machine Learning, and Data Science

---

## 📈 2026 Goals

- Contribute to major Open Source projects
- Publish more AI/Data Science research
- Master Machine Learning & Deep Learning
- Participate in GSoC and LFX Mentorship
- Build impactful Data Science projects
- Secure a Data Scientist role

---


## 📫 Connect With Me

- LinkedIn: https://www.linkedin.com/in/pawar-amrutha-124056292/
- Email: amruthapawar72@gmail.com
- GitHub: https://github.com/Amrutha-Pawar
